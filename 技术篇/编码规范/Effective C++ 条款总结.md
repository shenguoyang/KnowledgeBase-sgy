...
2）objected-oriented C++。类、封装、继承、多态
...
自我赋值很容易出现，比如用两个下标二维数组遍历对象时，主对角线上的元素很可能就被自我赋值。这一条条款提到了两个安全性：自我复制安全性和异常安全性。

先看自我赋值安全性，有如下代码：

**class** **Bitmap** { ... }; **class** **Widget** {     ... **private****:**     Bitmap***** pb; }; Widget**&** **operator****=**(**const** Widget**&** rhs) {     **delete** pb;     pb **=** **new** Bitmap(*****rhs.pb);     **return** *******this**; } 

如果this和rhs指向同一个对象，即自我复制，那delete pb析构了相当于析构了rhs.pb，后面一行对*rhs.pb的访问可能直接就coredump了。

其中一个解决办法时加入等同测试（identity test），代码如下：

Widget**&** **operator****=**(**const** Widget**&** rhs) {     **if** (**this** **==** **&**rhs) **return** *******this**;     **delete** pb;     pb **=** **new** Bitmap(*****rhs.pb);     **return** *******this**; } 

但是这个代码不是异常安全的，如果new出了问题（可能是malloc内存失败或者Bitmap的构造异常）,当前Widget对象就不能再安全地使用了。

如果使用临时变量来分配内存，等确认成功了再进行释放操作呢？考虑如下解决代码：

Widget**&** **operator****=**(**const** Widget**&** rhs) {     **if** (**this** **==** **&**rhs) **return** *******this**;  _// 这一行可以删掉了 _    Bitmap *****temp **=** **new** Bitmap(*****rhs.pb);     **delete** pb;     pb **=** temp;     **return** *******this**; } 

这样虽然解决了问题，但是多多少少看起来有点痛苦。使用所谓的copy and swap技术可以让代码逻辑简洁不少：

Widget**&** **operator=**(**const** Widget**&** rhs) {     Widget **temp**(rhs);     swap(rhs);     **return** ***this**; }
